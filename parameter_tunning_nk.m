...
%Code for Parameter tunning of bSpline parameters(n,k) for model 2 
clear all
close all
itrr=0;
...
%% all knots(n) and order(k) values for tunning 
nn=[8 9 10 11 12 13 14 15];
kk=[3 4 5 6 7];
cntt=0;
%loop for varying n parameters 
for inn=1:length(nn)
%loop for varying k parameters 
for ikk=1:length(kk)
    tic %starting time 
    n=nn(inn);k=kk(ikk);
    ...
    K= 0.5; F=0.8;           %Scalling and Combinition Factor
    %Population size
    ...
    tt=toc;
    %printing iteration count and time for each n and k with desired
    %accuracy
    fprintf('Total number of iteration for n=%d and k=%d is %d with time %f\n',n,k,jj,tt)
    cntt=cntt+1;
    %saving count data and iteration time
    value_data(cntt,:)=[n,k,jj,tt];
end
end
